...
def plot_data(title, data, column_prefixes=None, validate_only=False, nn_curve=False, clear_existing=True,
    ...
    print("HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH", column_prefixes, nn_curve)
            ...
            if not nn_curve:
                ...
                if not validate_only:
                    plt.plot(data.index, test, '-', linewidth=1, markersize=1,
                             label='Test {} {}'.format(column_prefix, y_label))
                    # plt.plot(data.index, val, '-', linewidth=1, markersize=1,
                    #          label='CV {} {}'.format(column_prefix, y_label))
                # print("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTXXX", title)
                # elapsed = data['{}_elapsed'.format(column_prefix)]
                # if not validate_only:
                #     plt.plot(data.index, elapsed, '-', linewidth=1, markersize=1,
                #              label='Time {} {}'.format(column_prefix, y_label))
                    # plt.plot(data.index, val, '-', linewidth=1, markersize=1,
                    #          label='CV {} {}'.format(column_prefix, y_label))
                else:
                    plt.plot(data.index, elapsed, '-', linewidth=1, markersize=1,
                             label='1{} {}'.format(column_prefix, y_label))
...
def plot_ga_data(problem_name, ga_files, output_dir, nn_curve=False):
    ...
    # y_label = 'Time Taken' if nn_curve else 'Fitness'
...
def plot_sa_data(problem_name, sa_files, output_dir, nn_curve=False):
    ...
    y_label = 'Time' if nn_curve else 'Fitness'
